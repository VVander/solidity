...
void ExecutionFramework::sendMessage(bytes const& _data, bool _isCreation, u256 const& _value)
{
	...
	auto const gasUsed = InitialGas - result.gas_left;
	auto const gasRefund = min(u256(result.gas_refund), gasUsed / refundRatio);

	m_gasUsed = gasUsed - gasRefund;
	...
	if (m_showMessages)
	{
		...
		cout << " gas used (without refund): " << gasUsed.str() << endl;
